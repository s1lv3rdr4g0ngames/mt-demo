...
#this gets closed in 4.7  https://github.com/godotengine/godot/pull/115649
...
func mt_seed(seeed : int):
	...
	for i in range(1, n):
		#1812433253 * (current index ^ ((current index shifted right 30 times) + index
		var temp = f * (mt_state_array[i-1] ^ (mt_state_array[i-1] >> (w-2))) + i
		#AND with max int
		mt_state_array[i] = temp & 0xffffffff
